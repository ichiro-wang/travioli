...
import { CheckUsernameQuery, DeleteAccountBody, GetProfileParams, UpdateProfileBody } from "../schemas/user.schemas.js";
 ...
 */
export const checkUsername = async (req: Request<{}, {}, {}, CheckUsernameQuery>, res: Response): Promise<void> => {
  try {
    const { username } = req.query;
    const { username: currentUserUsername } = req.user;

    // let UserService handle database interaction
    const { available, reason } = await userService.checkUsernameAvailability(username, currentUserUsername);

    if (!available) {
      const message = reason === "current" ? `@${username} is already your username` : `@${username} is already taken`;
 ...
 */
export const getUserProfile = async (req: Request<GetProfileParams>, res: Response): Promise<void> => {
  try {
 ...
 */
export const softDeleteAccount = async (req: Request<{}, {}, DeleteAccountBody>, res: Response): Promise<void> => {
  try {
    const { password } = req.body;
    const currentUser = req.user;

    const deletedUser = await userService.softDeleteUser(currentUser.id, password, currentUser.password);
